    ...
    for entry in entries.iter_mut() {
                    ...
                    .horizontal(|ui| {
                        ...
                        if resp.lost_focus() {
                            ...
                            if ui.input(|i| i.key_pressed(egui::Key::Enter))
                                ...
                                && name != original_name
                            {
                                *action = SidebarAction::ConfirmRenameEntry(
        ...
        match entry.kind {
            EntryKind::Directory => {
                ...
                // Force-open directory if the inline new entry targets it
                ...
                let (_toggle, header_inner, _body) = cs
                    .show_header(ui, |ui| {
                        let response = ui
                            .add(
                                egui::Label::new(format!("\u{1F4C1} {name}"))
                                    .sense(egui::Sense::click()),
                            )
                            .on_hover_cursor(egui::CursorIcon::PointingHand);
                        response.context_menu(|ui| {
                            show_new_entry_menu(ui, &path, &mut new_entry_request);
                            ui.separator();
                ...
                // Double-click on the header toggles expand/collapse
                if header_inner.inner.double_clicked() {
                    if let Some(mut state) =
                        egui::collapsing_header::CollapsingState::load(ui.ctx(), id)
                    {
